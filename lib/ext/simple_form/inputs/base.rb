# Allow every simple_form input to have options to define next_step as a native option (without resorting to data-attributes)
  ...
  module Inputs
    class Base
      ...
      def initialize(builder, attribute_name, column, input_type, options = {})
        ...
        validation_types = [:select, :radio_buttons, :boolean, :check_boxes]
        validation_types_with_change_event = [:select, :radio_buttons, :boolean]
        validation_event = validation_types_with_change_event.include?(@input_type) ? "change" : "input"
        @input_html_options = html_options_for(:input, input_html_classes).tap do |o|
          o[:data] ||= {}
          o[:data][:validation_target] = validation_types.include?(@input_type) ? @input_type&.to_s : "text"
          o[:data][:action] ||= ""
          o[:data][:action] << " #{validation_event}->step#handleClick #{validation_event}->validation#enableSubmit" unless options[:skip_validation]
          if options[:next_step]
            o[:data] ||= {}
            o[:data][:next_step] = options[:next_step]
            o[:data][:expected] = options[:expected]
            o[:data][:reveal] = options[:reveal]
          elsif options[:step_matrix]
            o[:data][:step_matrix] = options[:step_matrix].to_json
          elsif !options[:display_error_for].nil? # Can have a false value
            o[:data] ||= {}
            o[:data][:action] = "change->step#handleClick change->validation#handleError"
            o[:data][:display_error_for] = options[:display_error_for]
            o[:data][:reveal] = options[:reveal].to_json
          elsif !options[:reveal].nil? # Can have a false value
            o[:data] ||= {}
            o[:data][:reveal] = options[:reveal].to_json
          end
          if options[:skip_validation]
            o[:data] ||= {}
            o[:data][:validation_target] = ""
          end
          if options[:auto_submit]
            o[:data] ||= {}
            o[:data][:auto_submit] = true
          end
